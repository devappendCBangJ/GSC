...
parser.add_argument('--resume-checkpoint-path', default='/home/hi/Jupyter/MobileNetV2_pytorch_AIHub_BottleNeck/checkpoints/Temp/checkpoint.pth.tar', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
# 1-3) Transfer Learning - Pre Trained Model 사용 유무
parser.add_argument('--pretrained', dest='pretrained', action='store_true',
                    help='use pre-trained model')
# 1-4) Transfer Learning - Saved Path
parser.add_argument('--pretrained_model_path', default='/home/hi/Jupyter/MobileNetV2_pytorch_AIHub_BottleNeck/checkpoints/Temp/model_best.pth_tar', type=str, metavar='WEIGHT',   # 기존 이름 : '--pretrained_model_path' / # transfer learng 사용 시 : default : 'mobilenetv2_1.0-0c6065bc.pth'
                    help='path to pretrained weight (default: none)')

# 2-1) Dataset Path
parser.add_argument('-d', '--data', metavar='DIR', default='/media/hi/SK Gold P31/Korean_Emotion_Movie/compression/', # 기존 : default 존재하지 않음 -> "/home/hi/Datasets/Cifar10/" -> "/media/hi/SK Gold P31/Temp/" -> 현재 : "/media/hi/SK Gold P31/Korean_Emotion_Movie/"
...
parser.add_argument('--lr', '--learning-rate', default=0.06, type=float, # 기존 : 0.1 -> 현재 : 0.05 -> 잠깐 변경
...
parser.add_argument('--start-epoch', default=0, type=int, metavar='N',  # 기존 : 0 -> resume 시, 자동으로 불러옴
                    help='manual epoch number (useful on restarts)')
# 5-2) Train Epochs
parser.add_argument('--epochs', default=200, type=int, metavar='N', # 기존 : 90 -> 현재 : 150 -> 잠깐 변경
...
parser.add_argument('-c', '--checkpoint-path', default='checkpoints', type=str, metavar='PATH', # resume 시, 자동으로 불러옴
                    help='path to save checkpoint (default: checkpoints)')

# 7) Evaluate
parser.add_argument('-e', '--evaluate', dest='evaluate', default=False, # 기존 : action='store_true', default는 없음 -> 평가 시, True / 평가 안할 시, False
...
def trace_handler(prof):
    ...
    # prof.export_chrome_trace("./checkpoints/profiler/profile_cpu_gpu.json")
    # prof.export_stacks("./checkpoints/profiler/profile_cpu_gpu.txt", "self_cuda_time_total")
    # profiler.tensorboard_trace_handler("/home/hi/Jupyter/MobileNetV2_pytorch_AIHub_BottleNeck/checkpoints/profiler")
# ==============================================================
...
def main():
    ...
    if args.resume_checkpoint_path:
        if os.path.isfile(args.resume_checkpoint_path):
            print("=> loading checkpoint_path '{}'".format(args.resume_checkpoint_path))
            ...
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.resume_checkpoint_path, checkpoint['epoch']))
            # 2] 기존 Logger 열기
            args.checkpoint_path = os.path.dirname(args.resume_checkpoint_path)
            logger = Logger(os.path.join(args.checkpoint_path, 'log.txt'), title=title, resume=True)
        else:
            print("=> no checkpoint found at '{}'".format(args.resume_checkpoint_path))
    # (2) Resume 설정하지 않은 경우 : Start Checkpoint
    else:
        # 1] 새로운 Logger 열기
        logger = Logger(os.path.join(args.checkpoint_path, 'log.txt'), title=title)
    ...
    if args.evaluate:
        ...
        if os.path.isfile(args.pretrained_model_path):
            ...
            source_state = torch.load(args.pretrained_model_path) # 구조!!!
            # [1] .pt or .pth인 경우 모델 불러오기
            if args.pretrained_model_path[-4:] != ".tar":   # {1} Bang Env
        ...
        # 2] Weight 파일 존재하지 않는 경우 : 불러오지 않는다
            ...
            print("=> no weight found at '{}'".format(args.pretrained_model_path))

        # (2) Evaluate
        val_loss, prec1 = validate(val_loader, val_loader_len, model, criterion)    # {1} Bang Env
    ...
    #                       schedule=profiler.schedule(skip_first=0, wait=0, warmup=1, active=1, repeat=1),
    #                       on_trace_ready=trace_handler) as main_prof:
    for epoch in range(args.start_epoch, args.epochs):
        print('\nEpoch: [%d | %d]' % (epoch + 1, args.epochs))
        print("==========================================================================================") # {1} Bang Env

        # 1] Train -> loss, accuracy 출력
        # with profiler.record_function("Train"): # {1} Bang Env
        train_loss, train_acc = train(train_loader, train_loader_len, model, criterion, optimizer, epoch)

        # 2] Evaluate -> loss, accuracy 출력
        # with profiler.record_function("Val"): # {1} Bang Env
        val_loss, prec1 = validate(val_loader, val_loader_len, model, criterion)

        # with profiler.record_function("Logger"): # {1} Bang Env
        lr = optimizer.param_groups[0]['lr'] # 구조!!!
        # print("optimizer : ", optimizer)
        # print("optimizer.param_groups[0] : ", optimizer.param_groups[0])
        ...
        }, is_best, checkpoint=args.checkpoint_path)
        ...
        print((
            ...
            + f'Train Accuracy {train_acc:.04f} | '
            + f'Val Loss {val_loss:.04f} | '
            + f'Val Accuracy {prec1:.04f} | '
            + f'Best Val Accuracy {best_prec1:.04f} | '
            + f'lr {lr:.04f}'
        ))
        # ==============================================================

        # main_prof.step() # {1} Bang Env

    # (3) Save Values + Save Tensorboard (Close Logger / Close Writer / Save Checkpoint)
    logger.close()
    # ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
    logger.plot()
    savefig(os.path.join(args.checkpoint_path, 'log.eps'))
    # ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
    writer.close()
    print('Best accuracy:')
    print(best_prec1)
...
def train(train_loader, train_loader_len, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # 3]] Output 확률값 추출 (Train이므로 Gradient Update 필요)
